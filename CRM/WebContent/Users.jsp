...
<script type="text/javascript">
	...
	function caozuo(value, row, index) {
		return "<a href='javascript:void(0)' onclick='bianji(" + index
				+ ")'>编辑</a>   <a  href='javascript:void(0)' onclick='shanchu("
				+ index + ")'>删除</a>"
	}
	//锁定
	function suodings(value, row, index) {
		return "<a href='javascript:void(0)' onclick='suoding(" + index
				+ ")'>锁定</a>  <a href='javascript:void(0)' onclick='jiesuo("
				+ index + ")'>解锁</a>"
	}
	//删除
	...
	//新增保存
	function addbaocuen() {
		var accemail = document.getElementById("email2"); //获取email控件对象
		if (!accemail.value == "") {
			var reg = /^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/;
	...
	function guanli(value, row, index) {
		return "<a href='javascript:void(0)' onclick='guanlia(" + index
				+ ")'>管理</a>"
	}
	var uid = 0;
	...
	function updatebaocuen() {
		var accemail = document.getElementById("email3"); //获取email控件对象
		if (!accemail.value == "") {
			var reg = /^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/;
	...
	function chongzhi(value, row, index) {
		return "<a href='javascript:void(0)' onclick='ChongZhi(" + index
				+ ")'>重置密码</a>"
	}
	...
	function quanzhong(value, row, index) {
		return "<a href='javascript:void(0)' onclick='quanzhongguanli(" + index
				+ ")'>权重</a>";
	}
	var zid;
...
<body>
	<div id="tool">
		<form id="ff">
			<label for="name">名称:</label> <input class="easyui-textbox"
				style="width: 150px;" type="text" id="loginName" /> <label
				for="name">是否锁定:</label> <select id="isLoginData"
				class="easyui-combobox" name="dept" style="width: 150px;">
				<option value="">--请选择--</option>
				<option value="0">否</option>
				<option value="1">是</option>
			</select> <label for="name">创建开始时间:</label> <input id="cuanjiankaishi"
				type="text" class="easyui-datebox"> </input> <label for="name">创建结束时间:</label>
			<input id="cuanjianjieshu" type="text" class="easyui-datebox">
			</input> <label for="name">排序方式:</label> <select id="paixu"
				class="easyui-combobox" name="dept" style="width: 150px;">
				<option value="">--请选择--</option>
				<option value="0">创建时间</option>
				<option value="1">最后登录时间</option>
			</select> <br /> <a href="javascript:void(0)" class="easyui-linkbutton"
				onclick="init()" data-options="iconCls:'icon-search'">搜索</a> <a
				href="javascript:void(0)" class="easyui-linkbutton"
				onclick="xinzeng()" data-options="iconCls:'icon-add'">新增</a> <a
				href="javascript:void(0)" class="easyui-linkbutton"
				onclick="zidong()" data-options="iconCls:'icon-add'">变量自动分配</a>
		</form>
	...
	<div id="addwin" class="easyui-window" title="增加"
		...
		<div style="text-align: center; padding-top: 20px;">
			<table style="margin: auto;">
				<tr>
					...
					<td><input class="easyui-textbox" type="text" id="loginName2"
						name="loginName" /></td>
				</tr>
				...
				<tr>
					...
					<td><input class="easyui-textbox" type="text" id="email2"
						name="email" /></td>
				</tr>

				<tr>
					<td><label for="name">电话:</label></td>
					<td><input class="easyui-textbox" type="text" id="mtel2"
						name="mtel" " /></td>
				</tr>
			</table>
		</div>
		<div style="text-align: center; padding-top: 20px;">
			<a onclick="addbaocuen()" href="javascript:void(0)"
				class="easyui-linkbutton"">保存</a> <a onclick="addguanbi()"
				href="javascript:void(0)" class="easyui-linkbutton"">关闭</a>
		</div>
	</div>


	<div id="addjuese" class="easyui-window" title="角色"
		...
		<table>
			<thead>
				<tr>
					...
					<th><a onclick="juesetianjia()" href="javascript:void(0)"
						class="easyui-linkbutton"">>></a><br /> <br /> <a
						onclick="jueseshanchu()" href="javascript:void(0)"
						class="easyui-linkbutton"><<</a></th>
					<th>
	...
	<div id="updatewin" class="easyui-window" title="修改"
			...
			<table style="margin: auto;">
				<tr>
					...
					<td><input class="easyui-textbox" disabled="disabled"
						type="text" id="loginName3" name="loginName" /></td>
				</tr>

				<tr>
					<td><label for="name">邮箱:</label></td>
					<td><input class="easyui-textbox" type="text" id="email3"
						name="email" " /></td>
				</tr>

				<tr>
					<td><label for="name">电话:</label></td>
					<td><input class="easyui-textbox" type="text" id="mtel3"
						name="mtel" /></td>
				</tr>
			</table>
		...
		<div style="text-align: center; padding-top: 20px;">
			<a onclick="updatebaocuen()" href="javascript:void(0)"
				class="easyui-linkbutton"">保存</a> <a onclick="updateguanbi()"
				href="javascript:void(0)" class="easyui-linkbutton"">关闭</a>
		</div>
	</div>


	<div id="updatewinquanzhong" class="easyui-window" title="修改权重"
		style="width: 300px; height: 200px"
		data-options="iconCls:'icon-save',modal:true,closed:true">
		<div style="text-align: center; padding-top: 20px;">
			<table style="margin: auto;">
				<tr>
					...
					<td><input class="easyui-textbox" type="text" id="weight4"
						name="weight" /></td>
				</tr>
				<tr>
					<td><label for="name">备注:</label></td>
					<td><input class="easyui-textbox" type="text" id="remarks4"
						name="remarks" /></td>
				</tr>
			</table>
		</div>
		<div style="text-align: center; padding-top: 20px;">
			<a onclick="updatequanzhongbaocuen()" href="javascript:void(0)"
				class="easyui-linkbutton"">保存</a> <a
				onclick="updatequanzhongguanbi()" href="javascript:void(0)"
				class="easyui-linkbutton"">关闭</a>
		</div>
	</div>
	<div id="zidongfenpei" class="easyui-window" title="变量自动分配"
